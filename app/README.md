...
| `/job?id=N` | The full job record, a timeline built from the record's timestamps, listing and dispute details, a box that checks a pasted spec against the hash on chain, where the job stands with the compliance gate when the hook holds a module (the proof bound to it, current or stale and why, and for the client with a policy in this browser and `NEXT_PUBLIC_PROVER_URL` set, a button that binds one), the parties and what the hook's screening registry says about each, the settlement record once the job is settled (the compliance verdict and the module's reason for a refusal, the payee's screening, the payout routed, the reputation and validation writes; read from the indexer's `/jobs/:id/events`, so it needs `NEXT_PUBLIC_INDEXER_URL`), and every lifecycle action the connected wallet may take: set provider, set budget, fund (with automatic USDC approval, and a warning when the hook holds a module and this wallet has no policy on the registry, since that release would be refused), submit, evaluate (complete with a provider share, or reject, for the evaluator on the record when it is not the keeper), finalize, dispute, vote, apply a decision, lapse, list, buy or cancel a claim, reject, claim refund, withdraw, record expiry. A bound agent links to its DID on the agents page. |
| `/agents` | The agent side of the protocol: resolve any `did:aip` on this chain, or an agent id on this deployment's registry, to its DID document and the registration file behind it (owner, agent wallet, services, capabilities and prices, cross-registrations, warnings); and, with a wallet connected, register an ERC-8004 identity for it from a form that builds a registration file in the shape of [`docs/agent-card/schema.json`](../docs/agent-card/schema.json), stored on chain as a `data:` URI, at a URL the owner hosts, or left empty. The agent id is read from the receipt, never assumed, and the DID is derived from it. The agents this browser registered or added by id are kept locally per wallet, since the chain does not list an owner's agents. |
...
| `NEXT_PUBLIC_INDEXER_URL` | unset | Base URL of the indexer read API. When set, the dashboard reads the open and in-window counts from `/overview`, which counts them in the database rather than listing the rows, the network page shows `/status`, and the job page shows the settlement record from `/jobs/:id/events`. When unset, everything is read directly from the chain and the settlement record says how to get it. |
...
`npm test` runs the unit tests with Vitest: the phase derivation, the formatters, the chart aggregation, the action gates, the dialog focus order, the wallet inbox, the live statistics, the registration file (held to `docs/agent-card/schema.json` with Ajv), the agent id read from a receipt and the settlement record are pure modules under `src/lib` and are tested without a chain.
...
src/lib/agents.ts   The registration file built from the agents form, the data: URI it is stored as, the DID lookup, the wallet's local agent list (pure; tested against the card schema)
src/lib/registry.ts The ERC-8004 register ABIs and the agent id read from a receipt (pure)
src/lib/identity.ts Registering through the connected wallet, and resolving through @squaresdk/did-resolver
src/lib/settlement.ts The settlement record derived from the indexer's journal of a job (pure)
src/lib/screening.ts What the hook's screening registry says about a job's parties
