...
| [`skills/data/SKILL.md`](https://github.com/stellar/stellar-dev-skill/blob/main/skills/data/SKILL.md) | How the chain is read: `SquareClient.getEvents` over the kernel's own events, and the `latestLedgerCloseTime` each page carries as the clock the agent runtime's finalize runs on; `getHealth` for the latest ledger; `getLedgerEntries` on the account entry for a native balance, the SAC's balance entry for a contract's. |
