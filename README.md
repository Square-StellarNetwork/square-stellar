...
| [`skills/data/SKILL.md`](https://github.com/stellar/stellar-dev-skill/blob/main/skills/data/SKILL.md) | How the chain is read: `SquareClient.getEvents` over the kernel's own events, and the `latestLedgerCloseTime` each page carries as the clock the agent runtime's finalize runs on; `getHealth` for the latest ledger; `getLedgerEntries` on the account entry for a native balance, the SAC's balance entry for a contract's. The app that lands with [#39](https://github.com/Square-StellarNetwork/square-stellar/issues/39) adds `getLedgers` for the close time its countdowns run on and Horizon for an account's XLM. |
